...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 16 17:10:37 2020
...
def run_permutation(muts_df, df, emp_risk, emp_prot, num_runs):
    ...
    if stats.percentileofscore(dist_risks, emp_risk) < 1e-4 or stats.percentileofscore(dist_prots, emp_prot) < 1e-4:
            ret = run_permutation(muts_df, df, emp_risk, emp_prot, 1e5)
            ret1 = ret[0]
            ret2 = ret[1]
    elif stats.percentileofscore(dist_risks, emp_risk) < 1e-3 or stats.percentileofscore(dist_prots, emp_prot) < 1e-3:
            ret = run_permutation(muts_df, df, emp_risk, emp_prot, 1e5)
            ret1 = ret[0]
            ret2 = ret[1]
