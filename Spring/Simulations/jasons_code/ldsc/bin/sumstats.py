...
def _read_ref_ld(args, log):
    ...
    if args.baseline_ld_chr is not None:
        baseline_ld = _read_chr_split_files(args.baseline_ld_chr, args.baseline_ld, log,
                                    'reference panel baseline LD Score', ps.ldscore_fromlist)
        log.log(
            'Read reference panel paseline LD Scores for {N} SNPs.'.format(N=len(baseline_ld)))
        if not len(baseline_ld) == len(ref_ld):
            log.log("Error: baseline_ld length (%s) doesn't match ref ld length (%s)" % (len(baseline_ld), len(ref_ld)))
            raise
        ref_ld = smart_merge(ref_ld, baseline_ld)
...
def _read_annot(args, log):
    ...
    try:
        if args.ref_ld is not None:
            if args.baseline_ld is None:
                overlap_matrix, M_tot = _read_chr_split_files(args.ref_ld_chr, args.ref_ld, log,
                                                          'annot matrix', ps.annot, frqfile=args.frqfile)
            else:
                overlap_matrix, M_tot = _read_multiple_chr_split_files(None, [args.ref_ld, args.baseline_ld], log,
                                                          'annot matrix', ps.annot, frqfile=args.frqfile)
                if not baseline_M_tot == M_tot:
                    log.log("Error: baseline_M_tot (%s) doesn't match M_tot (%s)" % (baseline_M_tot, M_tot))
                    raise

        elif args.ref_ld_chr is not None:
            if args.baseline_ld_chr is None:
                overlap_matrix, M_tot = _read_chr_split_files(args.ref_ld_chr, args.ref_ld, log,
                                                              'annot matrix', ps.annot, frqfile=args.frqfile_chr)
            else:
                overlap_matrix, M_tot = _read_multiple_chr_split_files([args.ref_ld_chr, args.baseline_ld_chr], None, log,
                                                              'annot matrix', ps.annot, frqfile=args.frqfile_chr)
...
def _read_M(args, log, n_annot):
    ...
    if args.M:
        ...
        if args.ref_ld:
            ...
            if args.baseline_ld is not None:
                M_baseline_annot = ps.M_fromlist(
                    _splitp(args.baseline_ld), common=(not args.not_M_5_50))
                M_annot = np.concatenate(M_annot, M_baseline_annot)
            ...
            if args.baseline_ld_chr is not None:
                M_baseline_annot = ps.M_fromlist(
                    _splitp(args.baseline_ld_chr), _N_CHR, common=(not args.not_M_5_50))
                M_annot = np.concatenate((M_annot, M_baseline_annot), axis=1)
...
def _read_multiple_chr_split_files(chr_arg_array, not_chr_arg_array, log, noun, parsefunc, **kwargs):
    '''Read files split across 22 chromosomes (annot, ref_ld, w_ld).'''
    try:
...
def _read_ld_sumstats(args, log, fh, alleles=False, dropna=True, ref_ld=None, n_annot=None, M_annot=None, novar_cols=None, w_ld=None):

    if ref_ld is None or n_annot is None or M_annot is None or novar_cols is None or w_ld is None:
        ...
        log.log('Re-using ld stats files')
...
def estimate_h2(args, log):
    ...
    if len(args.h2) > 1:
        ...
        orig_M_annot, orig_ref_ld, orig_novar_cols = _check_variance(log, orig_M_annot, orig_ref_ld)
    ...
    for h2_fh in args.h2:
        ...
        M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols = _read_ld_sumstats(args, log, h2_fh, ref_ld=ref_ld, n_annot=n_annot, M_annot=M_annot, novar_cols=novar_cols, w_ld=w_ld)
        ...
        if args.overlap_annot:
            ...
            with open(args.out+'.results', 'a' if it > 0 else 'w') as op:
                df_results.to_csv(op, sep="\t", index=False, header=(it == 0), )
